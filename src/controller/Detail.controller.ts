...
export default class Detail extends BaseController
{
     ...
     */
    onShareEmailPress() {
        var oViewModel = this.getModel<sap.ui.model.json.JSONModel>("detailView");
    ...
    onListUpdateFinished(oEvent: sap.ui.base.Event) {
        var sTitle,
            ...
            oViewModel = this.getModel<sap.ui.model.json.JSONModel>("detailView");

        // only update the counter if the length is final
        if ((<sap.ui.model.odata.ODataListBinding>this.byId<sap.m.Table>("lineItemsList").getBinding("items")).isLengthFinal()) {
            if (iTotalItems) {
     ...
     * @private
     */
    private _onObjectMatched(oEvent: sap.ui.base.Event): void {
        var sObjectId =  oEvent.getParameter("arguments").objectId;
        this.getModel<sap.ui.model.odata.v2.ODataModel>().metadataLoaded().then(() => {
            var sObjectPath = this.getModel<sap.ui.model.odata.v2.ODataModel>().createKey("Objects", {
                ObjectID :  sObjectId
            });
    ...
    private _onBindingChange(): void {
        var oView = this.getView(),
            oElementBinding = <sap.ui.model.odata.v2.ODataContextBinding>oView.getElementBinding(<string><any>undefined);
        ...
        var sPath: string = (<any>oElementBinding).getPath(),
            ...
            oObject = (<sap.ui.model.odata.v2.ODataModel>oView.getModel(undefined)).getObject(sPath),
    ...
    private _onMetadataLoaded(): void {
        ...
        var iOriginalViewBusyDelay = this.getView().getBusyIndicatorDelay(),
            oViewModel = this.getModel<sap.ui.model.json.JSONModel>("detailView"),
            oLineItemTable = this.byId<sap.m.Table>("lineItemsList"),
